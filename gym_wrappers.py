...
class DiscreteSI(gym.ObservationWrapper):
    def __init__(self, env):
        ...
        self.observation_space = Box(low=0.,high=1.,shape=(73,),dtype=np.float32)
    ...
    def observation(self, obs):
        ...
        for key in addresses.keys():
            ...
            if "_x" in key or "_y" in key:
                ...
                res.append( (value/255)*2. - 1.)
            elif key == "invaders_left_count":
                value = int(obs[address])
                res.append( (1. - (value-1.)/36.))
            elif "row" in key:
                row = obs[address]
        ...
        #invaders direction
        res.append( -1. if (obs[24]>>1)&1 == 0 else 1.)

        #spaceship direction
        res.append( -1. if  obs[24]&1 == 0     else 1.)
...
class SIWrapper(gym.Wrapper):
    def __init__(self, env, random_starts = True,frame_skip=4,normalize_reward=True,negative_reward=False,episodic_life = False):
    ...
    def step(self, action):
        ...
        ale_interface: ALEInterface = self.env.unwrapped.ale
        self.render_buffer = []

        for t in range(self.frame_skip):
            obs, reward, terminated, truncated, info = self.env.step(action)
            self.render_buffer.append(self.env.render())
            total_reward += reward
        ...
        total_reward = total_reward if not self.normalize_reward else max(-1.,min(1.,total_reward))
